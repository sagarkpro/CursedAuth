...
/**
 * Two stateless filter chains. Chain 1 (order 1) is the public OAuth/OIDC surface and the
 * unauthenticated APIs — no bearer filter runs there. Chain 2 (order 2, default) protects
 * everything else (/userinfo and the authenticated user APIs) as a JWT resource server,
 * validating the RS256 access tokens we mint at /oauth/token.
 *
 * The default Spring Authorization Server endpoints (/oauth2/*) no longer exist: the
 * OAuth2AuthorizationServerConfigurer has been removed and the dependency dropped.
 */
@Configuration
